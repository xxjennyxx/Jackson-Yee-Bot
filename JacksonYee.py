...
import re
try:
    from intent import Loki_basic_information
    from intent import Loki_songs_and_films
except:
    from .intent import Loki_basic_information
    from .intent import Loki_songs_and_films
    ...
    if lokiRst.getStatus():
        for index, key in enumerate(inputLIST):
            for resultIndex in range(0, lokiRst.getLokiLen(index)):
                # basic_information
                if lokiRst.getIntent(index, resultIndex) == "basic_information":
                    resultDICT = Loki_basic_information.getResult(key, lokiRst.getUtterance(index, resultIndex), lokiRst.getArgs(index, resultIndex), resultDICT)

                # songs_and_films
                if lokiRst.getIntent(index, resultIndex) == "songs_and_films":
                    resultDICT = Loki_songs_and_films.getResult(key, lokiRst.getUtterance(index, resultIndex), lokiRst.getArgs(index, resultIndex), resultDICT)
...
def testIntent():
    # basic_information
    print("[TEST] basic_information")
    inputLIST = ['易烊千璽幾歲','易烊千璽是誰','易烊千璽姓什麼','易烊千璽幾年出生','易烊千璽的生日是什麼時候','易烊千璽的英文名字是什麼']
    testLoki(inputLIST, ['basic_information'])
    print("")

    # songs_and_films
    print("[TEST] songs_and_films")
    inputLIST = ['易烊千璽有哪些歌','易烊千璽出過哪些專輯','易烊千璽演唱過哪些歌曲']
    testLoki(inputLIST, ['songs_and_films'])
    print("")
...
if __name__ == "__main__":
    ...
    # 測試其它句子
    # resultDICT = runLoki(["易烊千璽的英文名字是什麼"])
    # resultDICT = runLoki(["易烊千璽幾歲"])
    # print(resultDICT)
